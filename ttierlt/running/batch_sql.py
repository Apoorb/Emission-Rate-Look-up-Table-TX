...
class SqlCmds:
    ...
    MAP_DISTRICT_ABB_FULL_NM_TXLED = {
        ...
        "crp": {"area_district": "Corpus Christi", "txled_active": True}, # Multiple counties in crp have TxLED.
        "dal": {"area_district": "Dallas", "txled_active": True},
        "ftw": {"area_district": "Fort Worth", "txled_active": True}, # Multiple counties in ftw have TxLED.
    ...
    def __init__(self, db_nm_, county_abb_):
        ...
        self.area_district = self.MAP_DISTRICT_ABB_FULL_NM_TXLED[county_abb_]["area_district"]
        self.use_txled = self.MAP_DISTRICT_ABB_FULL_NM_TXLED[county_abb_]["txled_active"]
    ...
    def test_sql_housekeeping(self):
        ...
        assert test_analysis_yr == self.analysis_year, "@analysis_year variable is not correctly set in MariaDB."
        assert abs(test_analysis_yr_todmix - self.analysis_year) <= 2, (
            ...
            "year")
        assert test_analysis_district == self.area_district, ("@analysis_district variable is not correctly set "
                                                              "in MariaDB.")
    ...
    def get_hour_mix_for_db_district(self, debug=DEBUG):
        ...
        self.cur.execute(
            ...
            """)
        if debug:
            self.hourmix = pd.read_sql(f"SELECT * FROM hourmix_{self.district_abb}", self.conn)
    ...
    def get_vmt_mix_for_db_district_weekday_closest_vmt_yr(self, debug=DEBUG):
        ...
        """
        self.cur.execute("FLUSH TABLES;")
        self.cur.execute(f"DROP TABLE  IF EXISTS vmtmix_weekday_{self.district_abb}_{self.analysis_year_todmix};")
        # FixMe: Use the district code (TXDOT_DistCode or TTI_DistCode) instead of TxDOT_Dist. String might have mis-
        #  matches.
        self.cur.execute(
            f"""
            ...
            """)
        if debug:
            self.vmtmix = pd.read_sql(f"SELECT * FROM  vmtmix_weekday_{self.district_abb}_{self.analysis_year_todmix} ",
                                      self.conn)
    ...
    def get_txled_for_db_district_year(self, debug=DEBUG):
        ...
        if self.use_txled:
            self.cur.execute("FLUSH TABLES;")
            self.cur.execute(f"DROP TABLE  IF EXISTS txled_long_{self.district_abb}_{self.analysis_year};")
            self.cur.execute(
                f"""
            ...
            """)
            self.cur.execute(f"SELECT DISTINCT yearid FROM txled_long_{self.district_abb}_{self.analysis_year};")
    ...
    def create_indices_before_joins(self):
        try:
            ...
            self.cur.execute("""CREATE INDEX IF NOT EXISTS  efidx3 ON emisrate (pollutantid, sourcetypeid, fueltypeid);""")
            self.cur.execute(f"""CREATE INDEX IF NOT EXISTS  houridx1 ON hourmix_{self.district_abb} (TOD);""")
            self.cur.execute(f"""CREATE INDEX IF NOT EXISTS  vmtidx1 ON vmtmix_weekday_{self.district_abb}_{self.analysis_year_todmix}
            ...
            print(mdberr)
            print("Run aggregate_emisrate_rateperdist, get_hour_mix_for_db_district, "
