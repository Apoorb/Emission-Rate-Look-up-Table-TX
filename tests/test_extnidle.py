"""
Tests to QAQC data processing from 01_batch_run_extnidle.py.
"""
import pytest
import re
import pandas as pd
...
def get_qaqc_tables_created_from_sql():
    ...
    search_pat = re.compile(".*extnidle_qaqc_from_orignal$")
    table_names = [
...
def get_py_sql_df_list(
    get_erlt_extnidle_2014b_data_py, get_qaqc_tables_created_from_sql, request
):
    group_area_yearid_monthid_py = get_erlt_extnidle_2014b_data_py.groupby(
        ["Area", "yearid", "monthid"]
    )
    ...
    try:
        ...
        print(
            err1,
            f"!!!Data for {request.param['grp_key']} does not exists in extnidle_erlt_intermediate!!!!",
        )
        raise
    try:
...
@pytest.mark.parametrize(
    "get_erlt_extnidle_2014b_data_py, get_py_sql_df_list",
    [
        (
...
@pytest.mark.parametrize(
    ...
    [
        {"data": "extnidle_erlt_intermediate", "fil_county": [district]}
