...
if __name__ == "__main__":
    ...
    emissionprocess1 = (
        emissionprocess
            .rename(columns={col: inflection.underscore(col) for col in
                             emissionprocess.columns})
    ...
    sourceusetype1 = (
        sourceusetype
            .rename(columns={col: inflection.underscore(col) for col in
                             sourceusetype.columns})
            .filter(items=["source_type_id", "source_type_name"])
    )

    mvs2014b_df = {
        "elp_2020_7": {"db": "mvs14b_erlt_elp_48141_2020_per_out",
                       "df": pd.DataFrame()},
        "elp_2030_7": {"db": "mvs14b_erlt_elp_48141_2030_per_out",
                       "df": pd.DataFrame()},
        "elp_2040_7": {"db": "mvs14b_erlt_elp_48141_2040_per_out",
                       "df": pd.DataFrame()},
        "elp_2050_7": {"db": "mvs14b_erlt_elp_48141_2050_per_out",
                       "df": pd.DataFrame()},
    }
    mvs3_df = {
    ...
    def process_mvs2014b_df(mvs2014b_df_, key_):
        ...
        idling_mvs2014b_1 = (
            idling_mvs2014b
            .merge(movesactivityoutput_2014b,
                   on=['yearID', 'monthID', 'hourID', 'linkID','sourceTypeID',
                       'fuelTypeID'],
                   how="left"
                   )
            .rename(columns={col: inflection.underscore(col) for col in
                             idling_mvs2014b.columns})
            .assign(rate_per_hour=lambda df: df.emission_quant / df.vmx,
                    moves=mvs_lab,
            ...
            .filter(items=[
                "moves", "year_id", "month", "day", "hour_id",
                "source_type_id", "fuel_type_id", "pollutant_id",
                "process_id", "process_name", "process_short_name",
                "source_type_name", "fuel_type_desc",
                "pollutant_name", "pollutant_short_name",
                "rate_per_hour"])
            .sort_values(by=["moves", "year_id", "month", "day", "hour_id",
                             "source_type_id", "fuel_type_id", "pollutant_id"])
            .reset_index(drop=True)
        )

        mean_count = idling_mvs2014b_1.groupby([
            "moves", "year_id", "month", "day", "hour_id", "source_type_id",
            "fuel_type_id", "pollutant_id",
            "process_id"]).rate_per_hour.count().mean()

        var_count = idling_mvs2014b_1.groupby([
            "moves", "year_id", "month", "day", "hour_id", "source_type_id",
            "fuel_type_id", "pollutant_id",
            "process_id"]).rate_per_hour.count().var()

        assert ((mean_count == 1) & (var_count == 0)), (
            "Look for multiple MOVES run in the output. Either filter them "
            "or delete them.")
        assert ~ idling_mvs2014b_1.isna().any().any(), "Check for missing " \
                                                   "values."
        assert ~ idling_mvs2014b_1.isna().any().any(), "Check for missing " \
                                                   "values."
    ...
    def process_mvs3_df(mvs3_df_, key_):
        ...
        idling_mvs3_1 = (
            ...
            .filter(items=[
                ...
                "rate_per_distance"])
            .sort_values(by=["moves", "year_id", "month", "day", "hour_id",
                             "source_type_id", "fuel_type_id",
                             "pollutant_id"])
            .reset_index(drop=True)
        ).rename(columns={"rate_per_distance": "rate_per_hour"})

        mean_count = idling_mvs3_1.groupby([
            "moves", "year_id", "month", "day", "hour_id", "source_type_id",
            "fuel_type_id", "pollutant_id",
            "process_id"]).rate_per_hour.count().mean()

        var_count = idling_mvs3_1.groupby([
            "moves", "year_id", "month", "day", "hour_id", "source_type_id",
            "fuel_type_id", "pollutant_id",
            "process_id"]).rate_per_hour.count().var()
        ...
        assert ((mean_count == 1) & (var_count == 0)), (
            "Look for multiple MOVES run in the output. Either filter them "
            "or delete them.")
        assert ~ idling_mvs3_1.isna().any().any(), "Check for missing " \
                                                   "values."
        assert ~ idling_mvs3_1.isna().any().any(), "Check for missing " \
                                                   "values."
    ...
    for key in mvs3_df.keys():
        mvs2014b_df[key]["df"] = process_mvs2014b_df(
            mvs2014b_df_=mvs2014b_df, key_=key)

        mvs3_df[key]["df"] = process_mvs3_df(
            mvs3_df_=mvs3_df, key_=key)

    final_df = pd.concat(
        [value2 for value in mvs2014b_df.values()
         for key2, value2 in value.items() if key2 == "df"]
        + [value2 for value in mvs3_df.values()
           for key2, value2 in value.items() if key2 == "df"])

    final_df_agg = (
        final_df
            .groupby(["moves", "year_id", "month", "day", "source_type_id",
                      "source_type_name", "fuel_type_id", "fuel_type_desc",
                      "pollutant_id", "pollutant_short_name"])
            .agg(rate_per_hour=("rate_per_hour", "sum"))
            .reset_index()
    )

    temp_per_diff = final_df_agg.set_index([
        "year_id", "month", "day", "source_type_id",
        "source_type_name", "fuel_type_id", "fuel_type_desc",
        "pollutant_id", "pollutant_short_name","moves"]).unstack()

    temp_per_diff["per_diff"] = np.round(
        ((temp_per_diff[("rate_per_hour", "MOVES 3")]
          - temp_per_diff[("rate_per_hour", "MOVES 2014b")])
         / temp_per_diff[("rate_per_hour", "MOVES 2014b")]) * 100
        , 2)

    temp_per_diff = (
        temp_per_diff.reset_index().drop("rate_per_hour", axis=1)
        .droplevel(1, axis=1)
    )
    final_df_agg_1 = final_df_agg.merge(
        temp_per_diff,
        on = ["year_id", "month", "day", "source_type_id",
                  "source_type_name", "fuel_type_id", "fuel_type_desc",
                  "pollutant_id", "pollutant_short_name"]
    )
