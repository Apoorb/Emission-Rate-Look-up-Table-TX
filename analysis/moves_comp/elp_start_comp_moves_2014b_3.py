...
if __name__ == "__main__":
    ...
    for key in mvs3_df.keys():
        for mvs_lab, mvs_df in zip(
            ...
            mean_count = start_mvs1.groupby([
                "moves", "year_id", "month", "day", "hour_id", "source_type_id",
                "fuel_type_id", "pollutant_id",
                "process_id"]).rate_per_start.count().mean()

            var_count = start_mvs1.groupby([
                "moves", "year_id", "month", "day", "hour_id", "source_type_id",
                "fuel_type_id", "pollutant_id",
                "process_id"]).rate_per_start.count().var()

            assert ((mean_count == 1) & (var_count == 0)), (
                "Look for multiple MOVES run in the output. Either filter them "
                "or delete them.")
            assert ~ start_mvs1.isna().any().any(), "Check for missing " \
                                                      "values."
            assert ~ start_mvs1.isna().any().any(), "Check for missing " \
                                                      "values."
    ...
    temp_per_diff["per_diff"] = np.round(
        ((temp_per_diff[("rate_per_start", "MOVES 3")]
         ...
         / temp_per_diff[("rate_per_start", "MOVES 2014b")]) * 100
        , 2)

    temp_per_diff = (
        temp_per_diff.reset_index().drop("rate_per_start", axis=1)
        .droplevel(1, axis=1)
    )
