"""
Compare El Paso 4 year of extended idling emission rates between MOVES 2014b
and 3 by
    hour (9),
...
if __name__ == "__main__":
    ...
    out_path = os.path.join(out_dir, "extnidle_mvs_2014b_3.csv")
    ...
    for key in mvs3_df.keys():
        for mvs_lab, mvs_df in zip(
                ["MOVES 2014b",  "MOVES 3"], [mvs2014b_df,  mvs3_df]):
            conn = connect_to_server_db(database_nm=mvs_df[key]["db"])
            extnidle_mvs = pd.read_sql(
                f"""SELECT * FROM rateperhour 
            ...
            assert ((mean_count == 1) & (var_count == 0)), (
                ...
                "or delete them.")
            assert ~ extnidle_mvs1.isna().any().any(), "Check for missing " \
                                                      "values."
            assert ~ extnidle_mvs1.isna().any().any(), "Check for missing " \
                                                      "values."
            mvs_df[key]["df"] = extnidle_mvs1
