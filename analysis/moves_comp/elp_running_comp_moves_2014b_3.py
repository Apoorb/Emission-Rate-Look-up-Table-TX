...
if __name__ == "__main__":
    ...
    avgspeedbin1 = (
        ...
        .filter(items=["avg_speed_bin_id", "avg_bin_speed",
                       "avg_speed_bin_desc"])
    )
        ...
        for mvs_lab, mvs_df in zip(
            ...
            running_mvs1 = (
                ...
                .filter(items=[
                    ...
                    "source_type_id",
                    "fuel_type_id", "pollutant_id", "avg_speed_bin_id",
                    "road_type_id", "process_id",
                    "avg_bin_speed", "avg_speed_bin_desc", "process_name",
                    "process_short_name", "source_type_name", "fuel_type_desc",
                    "road_desc", "pollutant_name", "pollutant_short_name",
                    "rate_per_distance"])
            )

            mean_count = running_mvs1.groupby([
                "moves", "year_id", "month", "day", "hour_id", "source_type_id",
                "fuel_type_id", "pollutant_id", "avg_speed_bin_id",
                "road_type_id", "process_id"]).rate_per_distance.count().mean()

            var_count = running_mvs1.groupby([
                "moves", "year_id", "month", "day", "hour_id", "source_type_id",
                "fuel_type_id", "pollutant_id", "avg_speed_bin_id",
                "road_type_id", "process_id"]).rate_per_distance.count().var()

            assert ((mean_count == 1) & (var_count == 0)), (
                "Look for multiple MOVES run in the output. Either filter them "
                "or delete them.")
            assert ~ running_mvs1.isna().any().any(), "Check for missing " \
                                                      "values."
    ...
    final_df_agg = (
        final_df
        .groupby(["moves", "year_id", "month", "day", "road_type_id",
                  "road_desc",
                  "source_type_id", "source_type_name", "fuel_type_id",
                  "fuel_type_desc", "pollutant_id", "pollutant_short_name",
                  "avg_speed_bin_id", "avg_bin_speed", "avg_speed_bin_desc"])
        .agg(rate_per_distance=("rate_per_distance", "sum"))
        .reset_index()
    )

    temp_per_diff = final_df_agg.set_index([
        "year_id", "month", "day", "road_type_id",
        "road_desc", "source_type_id", "source_type_name", "fuel_type_id",
        "fuel_type_desc", "pollutant_id", "pollutant_short_name",
        "avg_speed_bin_id", "avg_bin_speed", "avg_speed_bin_desc","moves"]
    ).unstack()

    temp_per_diff["per_diff"] = np.round(
        ((temp_per_diff[("rate_per_distance", "MOVES 3")]
          - temp_per_diff[("rate_per_distance", "MOVES 2014b")])
         / temp_per_diff[("rate_per_distance", "MOVES 2014b")]) * 100
        , 2)

    temp_per_diff = (
        temp_per_diff.reset_index().drop("rate_per_distance", axis=1)
        .droplevel(1, axis=1)
    )
