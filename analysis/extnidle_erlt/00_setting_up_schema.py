...
from ttierlt.extnidle.extnidle_batch_sql import create_extnidle_table_in_db
...
if delete_if_exists:
    delete_if_exists_user_input = input(
        "Do you want to loose the data from previous runs and create a fresh extnidle_erlt_intermediate table?(y/n)"
    )
    if delete_if_exists_user_input.lower() == "y":
        create_extnidle_table_in_db(delete_if_exists=delete_if_exists_user_input)
