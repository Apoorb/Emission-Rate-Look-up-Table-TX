...
if __name__ == "__main__":
    ...
    path_qaqc_manual_before_yr_interpol = os.path.join(
        PATH_INTERIM_IDLING, "qacqc_manual_yr_interpol_idling_erlt_intermediate.xlsx"
    )
    path_qaqc_py_after_yr_interpol = os.path.join(
        PATH_INTERIM_IDLING, "qacqc_py_yr_interpol_idling_erlt_intermediate.xlsx"
    )
    ...
    erlt_df_2014b_py_yr_iterpolated.to_sql(
        "idling_erlt_intermediate_yr_interpolate",
        con=engine,
        if_exists="replace",
        index=False,
    )
