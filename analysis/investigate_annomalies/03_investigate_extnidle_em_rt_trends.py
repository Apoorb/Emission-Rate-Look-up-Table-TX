...
erlt_df_2014b_py_1 = (
    erlt_df_2014b_py
    .set_index(['Area', 'yearid', 'monthid', 'Processtype'])
    .stack()
    .reset_index()
...
erlt_df_2014b_py_2 = (
    ...
    .assign(
        emis_rt_shift_down=lambda df: (
            df.groupby(['pollutant', 'Area', 'monthid', 'Processtype'])
            .emis_rt.shift(1)
        ),
...
erlt_df_2014b_py_annual = (
    erlt_df_2014b_py_1
    .groupby(['Area', 'yearid', 'Processtype', 'pollutant'])
    .emis_rt.max()
    .reset_index()
...
def debug_raw_extndidlerate_tb(db_yr1, db_yr2, suffixes):
    ...
    extndidle_district_yr1 = pd.read_sql("SELECT * FROM extnidlerate",
                                      district_yr1_db_conn)
    district_yr1_db_conn.close()
    extndidle_district_yr1_fil = (
        extndidle_district_yr1
        .loc[lambda df:(~ df.pollutantID.isna())]
    )
    district_yr2_db_conn = connect_to_server_db(db_yr2)
    extndidle_district_yr2 = pd.read_sql("SELECT * FROM extnidlerate", district_yr2_db_conn)
    district_yr2_db_conn.close()
    extndidle_district_yr2_fil = (
        extndidle_district_yr2
        .loc[lambda df: (~ df.pollutantID.isna())]
    )
    ...
    extndidle_district_yr1_yr2 = (
        pd.merge(
            extndidle_district_yr1_fil,
            extndidle_district_yr2_fil,
            on=['Area', 'monthid', 'hourID', 'pollutantID', 'processid',
                'Processtype', 'sourceTypeID', 'sourceTypeID', 'fuelTypeID'],
            suffixes=suffixes
        )
        .assign(
            ...
            is_discrepancy_emission=lambda df: (df.rateperhour_diff > 0.00001).astype(int),
            tot_emisfact_yr1=lambda df: df.groupby([
                'Area', 'monthid', 'pollutantID',
                'Processtype'])[f"emisFact{yr1_suf}"].transform(sum),
            tot_emisfact_yr2=lambda df: df.groupby([
                'Area', 'monthid', 'pollutantID',
                'Processtype'])[f"emisFact{yr2_suf}"].transform(sum),
            tot_emisfact_diff=lambda df: df.tot_emisfact_yr2
                                         - df.tot_emisfact_yr1,
            is_discrepency_tot_diff=lambda df: (df.tot_emisfact_diff >
                                                0.00001).astype(int)
        )
        .sort_values(["pollutantID", 'Area',
                          'monthid',
                          'hourID', 'Processtype'],
...
path_out_running_debug = os.path.join(PATH_PROCESSED,
                                   ...
                                   "extnidle_const_rate_debug.xlsx")
