...
erlt_df_2014b_py_1 = (
    erlt_df_2014b_py.set_index(["Area", "yearid", "monthid", "hourid", "period"])
    .stack()
    .reset_index()
...
erlt_df_2014b_py_2 = erlt_df_2014b_py_1.assign(
    emis_rt_shift_down=lambda df: (
        df.groupby(["pollutant", "Area", "monthid", "hourid", "period"]).emis_rt.shift(
            1
        )
    ),
...
erlt_df_2014b_py_annual = (
    erlt_df_2014b_py_1.groupby(["Area", "yearid", "pollutant"])
    .emis_rt.max()
    .reset_index()
...
def debug_raw_idlerate_tb(
    db_yr1, db_yr2, suffixes, pol_map={33: "NO2", 3: "NOX", 31: "SO2", 98: "CO2EQ"}
):
    ...
    idlert_district_yr1_fil = idlert_district_yr1.assign(
    ...
    idlert_district_yr2_fil = idlert_district_yr2.assign(
        pollutant=lambda df: df.pollutantid.map(pol_map)
    ).loc[lambda df: (~df.idlerate.isna()) & (~df.pollutant.isna())]
    ...
    idlert_district_yr1_yr2 = (
        ...
        .assign(
            ...
            tot_emisfact_yr1=lambda df: df.groupby(
                ["Area", "monthid", "period", "hourid", "countyid", "pollutant"]
...
path_out_idle_debug = os.path.join(
    PATH_PROCESSED, "debug_inconsistent_patterns", "idle_NO2_2038_rt_inc_debug.xlsx"
)
