...
erlt_df_2014b_py_1 = (
    erlt_df_2014b_py
    .set_index(['Area', 'yearid', 'monthid', 'funclass', 'avgspeed'])
    .stack()
    .reset_index()
    .rename(columns={"level_5": "pollutant", 0: "emis_rt"})
    .sort_values(['pollutant', 'Area', 'monthid', 'funclass', 'avgspeed',
                  'yearid'])
)
...
erlt_df_2014b_py_2 = (
    ...
    .assign(
        emis_rt_shift_down=lambda df: (
            df.groupby(['pollutant', 'Area', 'monthid', 'funclass', 'avgspeed'])
            .emis_rt.shift(1)
        ),
...
erlt_df_2014b_py_2_dal_so2_44_46 = (
    erlt_df_2014b_py_2.loc[lambda df: (df.Area == "Dallas")
                                      & (df.pollutant == "SO2")
...
erlt_df_2014b_py_annomalies_dal = erlt_df_2014b_py_annomalies.loc[lambda df:
df.Area == "Dallas"]

erlt_df_2014b_py_annomalies_ftw = erlt_df_2014b_py_annomalies.loc[lambda df:
df.Area == "Fort Worth"]


erlt_df_2014b_py_annomalies[lambda df: df.pollutant == "NAPTH"].eval("emis_rt - emis_rt_shift_down") / 0.00004
problem_area_years_pol = erlt_df_2014b_py_annomalies.groupby(["Area"]).agg(
    yearid=("yearid", set),
    pollutant=("pollutant", set))

erlt_df_2014b_py_annual = (
    erlt_df_2014b_py_1
    .groupby(['Area', 'yearid', 'funclass', 'avgspeed', 'pollutant'])
    .emis_rt.max()
    .reset_index()
    .sort_values(['pollutant', 'Area', 'yearid', 'funclass', 'avgspeed'])
    .assign(
        emis_rt_shift_down=lambda df: (
            df.groupby(['pollutant', 'Area', 'funclass', 'avgspeed'])
            .emis_rt.shift(1)
        ),
        diff_emis_rt=lambda df: df.emis_rt - df.emis_rt_shift_down,
...
erlt_df_2014b_py_annual_annomalies_sets = erlt_df_2014b_py_annual_annomalies\
    .groupby(["pollutant", "Area"]).agg(
...
def debug_raw_runningrate_tb(db_yr1, db_yr2,
                             suffixes,
                             pol_map = {31: "SO2", 98: "CO2EQ"}):
    district_yr1_db_conn = connect_to_server_db(db_yr1)
    runrt_district_yr1 = pd.read_sql("SELECT * FROM emisrate",
                                      district_yr1_db_conn)
    ...
    runrt_district_yr2 = pd.read_sql("SELECT * FROM emisrate",
                                     district_yr2_db_conn)
    ...
    runrt_district_yr1_yr2 = (
        pd.merge(
            runrt_district_yr1_fil,
            runrt_district_yr2_fil,
            on=['Area', 'monthid', 'hourid', 'Period', 'roadtypeid',
                'Funclass',
                'pollutantid', 'pollutant',
                'sourcetypeid', 'fueltypeid', 'avgSpeedBinID','avgspeed'],
            suffixes=suffixes
        )
        .assign(
            ERate_diff=lambda df: df[f"ERate{yr2_suf}"]
                                     - df[f"ERate{yr1_suf}"],
            emisFact_diff=lambda df: df[f"emisFact{yr2_suf}"]
                                     - df[f"emisFact{yr1_suf}"],
            is_discrepancy=lambda df: (df.emisFact_diff > 0.0000001).astype(
                int),
            is_erate_discrepency=lambda df: (df.ERate_diff > 0.0000001).astype(
                int),
            ...
            is_discrepency_tot_diff=lambda df: (df.tot_emisfact_diff >
                                                0.0000001).astype(int),
            is_stypemix_increasing=lambda df:((df[f"stypemix{yr2_suf}"]
                - df[f"stypemix{yr1_suf}"]) > 0.0000001).astype(int)
        )
        .sort_values(["is_discrepency_tot_diff", "pollutant", 'Area',
                      'Funclass',
                      'avgSpeedBinID', 'monthid', 'hourid'],
...
dal_issue2_so2 = (dal_issue2.loc[lambda df: (df.pollutant == "SO2")
                                           & (df.avgspeed == 2.5)]
    .groupby(["roadtypeid", "pollutant"])
    .agg(emisFact_44=("emisFact_44", "sum"),
...
path_out_running_debug = os.path.join(PATH_PROCESSED,
