...
erlt_df_2014b_py_1 = (
    erlt_df_2014b_py
    .rename(columns={"VehicleType": "vehicletype", "FUELTYPE": "fueltype"})
    .set_index(['Area', 'yearid', 'monthid', 'vehicletype', 'fueltype'])
    .stack()
    .reset_index()
    .rename(columns={"level_5": "pollutant", 0: "emis_rt"})
    .sort_values(['pollutant', 'Area', 'monthid', 'vehicletype', 'fueltype',
                  'yearid'])
)
...
erlt_df_2014b_py_2 = (
    ...
    .assign(
        emis_rt_shift_down=lambda df: (
            df.groupby(['pollutant', 'vehicletype', 'fueltype', 'Area', 'monthid'])
            .emis_rt.shift(1)
        ),
...
erlt_df_2014b_py_annomalies = (
    erlt_df_2014b_py_2
    .loc[lambda df: df.diff_emis_rt > 0.0001]
)


erlt_df_2014b_py_annual = (
    erlt_df_2014b_py_1
    .groupby(['Area', 'yearid', 'vehicletype', 'fueltype', 'pollutant'])
    .emis_rt.max()
    .reset_index()
    .sort_values(['pollutant', 'Area', 'yearid', 'vehicletype', 'fueltype'])
    .assign(
        emis_rt_shift_down=lambda df: (
            df.groupby(['pollutant', 'Area', 'vehicletype', 'fueltype'])
            .emis_rt.shift(1)
        ),
        diff_emis_rt=lambda df: df.emis_rt - df.emis_rt_shift_down,
    )
...
def debug_raw_startrate_tb(db_yr1, db_yr2,
    ...
    startrt_district_yr1_yr2 = (
        pd.merge(
            startrt_district_yr1_fil,
            startrt_district_yr2_fil,
            on=['Area', 'monthid', 'hourid', 'sourcetypeid',
                'fueltypeid',
                'pollutantid', 'VehicleType', 'FuelType'],
            suffixes=suffixes
        )
        .assign(
            ...
            tot_emisfact_yr1=lambda df: df.groupby([
                ...
                'pollutantid'])[f"emisFact{yr1_suf}"].transform(sum),
            tot_emisfact_yr2=lambda df: df.groupby([
                'Area', 'monthid', 'VehicleType', "FuelType",
                'pollutantid'])[f"emisFact{yr2_suf}"].transform(sum),
        ...
        .sort_values(["is_discrepency_tot_diff", "pollutantid", 'Area',
                      'sourcetypeid',
                      'fueltypeid'],
...
path_out_running_debug = os.path.join(PATH_PROCESSED,
                                   ...
                                   "start_refuse_2026_28_debug.xlsx")
