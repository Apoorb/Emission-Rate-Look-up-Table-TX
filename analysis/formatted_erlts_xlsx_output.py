...
if __name__ == "__main__":
    path_out_running = os.path.join(PATH_PROCESSED_RUNNING,
                                    "running_erlt_2014b.xlsx")
    path_out_start = os.path.join(PATH_PROCESSED_STARTS,
                                  "start_erlt_2014b.xlsx")
    path_out_idling = os.path.join(PATH_PROCESSED_IDLING,
                                   "idling_erlt_2014b.xlsx")
    path_out_extnidle = os.path.join(PATH_PROCESSED_EXTNIDLE,
                                     "extnidle_erlt_2014b.xlsx")
    ...
    running_2014b_1 = (
        ...
        .filter(items=[
            'Area', 'Year', 'Road Type ID', 'Road Description', 'Average Speed',
            'CO', 'NOX', 'SO2','NO2', 'CO2EQ', 'VOC', 'PM10', 'PM25', 'BENZ',
            'NAPTH', 'BUTA', 'FORM', 'ACTE', 'ACROL', 'ETYB', 'DPM', 'POM'
        ])
        .sort_values(by=[
            'Area', 'Year', 'Road Type ID', 'Road Description', 'Average Speed']
        )
    ...
    start_2014b_1 = (
        ...
        .rename(columns={
            ...
            'FUELTYPE': 'Fuel Type'})
        .filter(items=[
            'Area', 'Year', 'Source Type ID', 'Fuel Type ID', 'Source Type',
            'Fuel Type',
            'CO', 'NOX', 'SO2','NO2', 'CO2EQ', 'VOC', 'PM10', 'PM25', 'BENZ',
            'NAPTH', 'BUTA', 'FORM', 'ACTE', 'ACROL', 'ETYB', 'DPM', 'POM'
        ])
    ...
    extnidle_2014b_1=(
        ...
        .filter(items=[
            'Area', 'Year', 'Process Type ID', 'Process Type',
            'CO', 'NOX', 'SO2', 'NO2', 'CO2EQ', 'VOC', 'PM10', 'PM25',
            'BENZ',
            'NAPTH', 'BUTA', 'FORM', 'ACTE', 'ACROL', 'ETYB', 'DPM', 'POM'
            ])
        )
    ...
    with pd.ExcelWriter(path_out_running) as writer:
        for area in running_2014b_1.Area.unique():
            erlt_df_2014b_py_1_fil = running_2014b_1.loc[
                lambda df: df.Area == area]
            erlt_df_2014b_py_1_fil.to_excel(
                writer, sheet_name=area, index= False)

    with pd.ExcelWriter(path_out_start) as writer:
        for area in start_2014b_1.Area.unique():
            start_2014b_1_fil = start_2014b_1.loc[
                lambda df: df.Area == area]
            start_2014b_1_fil.to_excel(
                writer, sheet_name=area, index= False)

    with pd.ExcelWriter(path_out_idling) as writer:
        for area in idling_2014b_1.Area.unique():
            idling_2014b_1_fil = idling_2014b_1.loc[
                lambda df: df.Area == area]
            idling_2014b_1_fil.to_excel(
                writer, sheet_name=area, index= False)

    with pd.ExcelWriter(path_out_extnidle) as writer:
        for area in extnidle_2014b_1.Area.unique():
            extnidle_2014b_1_fil = extnidle_2014b_1.loc[
                lambda df: df.Area == area]
            extnidle_2014b_1_fil.to_excel(
                writer, sheet_name=area, index= False)